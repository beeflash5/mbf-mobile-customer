...
class _ParcelPageState extends ConsumerState<ParcelPage> {
  ...
  Future<void> _track(String code) async {
    ...
    switch (result) {
      case TrackOrderSuccess(:final order):
        context.pushWidget(OrderDetailsPage(
              order: order,
  ...
  Widget build(BuildContext context) {
    final asyncTracking =
        ref.watch(parcelTrackingControllerProvider(widget.vendorType.id));
      ...
      body: SmartRefresher(
        ...
        child: VStack([
          ...
          CustomButton(
            ...
            onPressed: () =>
                context.pushRoute('/parcel/new', extra: widget.vendorType),
          ).wFull(context).px20(),
          UiSpacer.verticalSpace(),
          RecentOrdersView(
