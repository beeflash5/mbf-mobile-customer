...
class AppStrings {
  ...
  static String get wa_support => env('wa_support') ?? "";
  //
  static bool get isSingleVendorMode => env('isSingleVendorMode') == "1";
  static bool get canScheduleTaxiOrder =>
      envMap('taxi')['canScheduleTaxiOrder'] != null
          ? (envMap('taxi')['canScheduleTaxiOrder'] == "1")
          : false;
  static int get taxiMaxScheduleDays =>
      (envMap('taxi')['taxiMaxScheduleDays'].toString().toInt()) ?? 2;
  ...
  static double get down_payment => double.parse("${env('down_payment') ?? 0}");
  static double get reschedule_fee =>
      double.parse("${env('reschedule_fee') ?? 0}");
  static double get auto_cancel_fee =>
      double.parse("${env('auto_cancel_fee') ?? 0}");
  ...
  //Social media logins
  static bool get googleLogin => envMap('auth')['googleLogin'] ?? false;
  static bool get appleLogin => envMap('auth')['appleLogin'] ?? false;
  static bool get facebbokLogin => envMap('auth')['facebbokLogin'] ?? false;
  static bool get qrcodeLogin => envMap('auth')['qrcodeLogin'] ?? false;
  ...
  static double get categoryTextSize {
    ...
    return double.parse(
      (envMap('ui')['categorySize']["text"]['size'] ?? 12.00).toString(),
    );
  }
  ...
  static int get categoryPerRow {
    try {
      ...
      return int.parse((envMap('ui')['categorySize']["row"] ?? 4).toString());
    } catch (e) {
      return 3;
    }
  }
  ...
  static bool get categoryStyleGrid {
    try {
      if (envMap('ui')["categoryStyle"] == null) {
        return true;
      }
      String style = envMap('ui')['categoryStyle'].toString().toLowerCase();
      return style == "grid";
    } catch (e) {
      return true;
    }
  ...
  static bool get searchGoogleMapByCountry {
    if (envMap('ui')["google"] == null) {
      return false;
    }
    return envMap('ui')['google']["searchByCountry"] ?? false;
  }
  ...
  static String get searchGoogleMapByCountries {
    ...
    return envMap('ui')['google']["searchByCountries"] ?? "";
  }
  ...
  /// Null/type-safe access to a nested config object (auth, taxi, ui, ...).
  /// env() may return "" (settings not loaded yet), null (key missing) or a
  /// non-map value — none safe to index with a string. Always hand back a Map
  /// so callers can do envMap('auth')['googleLogin'] without crashing.
  static Map envMap(String ref) {
    final value = env(ref);
