...
class ForgotPasswordViewModel extends MyBaseViewModel {
  ...
  processFirebaseForgotPassword(String phoneNumber) async {
    ...
    await FirebaseAuth.instance.verifyPhoneNumber(
      ...
      verificationFailed: (FirebaseAuthException e) {
        ...
        AlertService.error(title: "Forgot Password".tr(), text: msg);
      },
  ...
  //
  processCustomForgotPassword(String phoneNumber, String? email) async {
    setBusy(true);
    try {
      await _authRequest.sendOTP(phoneNumber, email);
      setBusy(false);
      showVerificationEntry();
    } catch (error) {
      setBusy(false);
  ...
  void showVerificationEntry() async {
    //
    await viewContext.push((context) {
  ...
  void verifyCustomOTP(String smsCode) async {
    ...
    try {
      ...
      AlertService.error(title: "Verifikasi".tr(), text: "$error");
      // viewContext.showToast(msg: "$error", bgColor: Colors.red);
    }
    //
    setBusyForObject(firebaseVerificationId, false);
