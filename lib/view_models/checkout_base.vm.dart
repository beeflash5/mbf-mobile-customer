...
class CheckoutBaseViewModel extends PaymentViewModel {
  ...
  TextEditingController guest_count = TextEditingController();
  ...
  List<String> date_full = [];
  List<String> time_full = [];
  ...
  getTableUse() async {
    ...
    try {
      ...
      for (int i = 1; i <= vendor!.qty_tables!; i++) {
  ...
  getTimeUse() async {
    ...
    if (checkout == null) return;
    //
    setBusyForObject(getTimeUse, true);
    try {
      final response = await vendorRequest.vendorGetTimeUse(
        vendor!.id,
        checkout!.deliverySlotDate,
      );

      time_full.clear();
      time_full = List<String>.from(response.body);

      print("testing ${time_full}");

      notifyListeners();
    } catch (error) {
      print("Error Getting Vendor Details ==> $error");
    }
    setBusyForObject(getTimeUse, false);
  }
  ...
  togglePickupStatus(bool? value) {
    ...
    isScheduled = value == true ? false : true;
  ...
  placeOrder({bool ignore = false}) async {
    ...
    if (isScheduled && checkout!.deliverySlotDate.isEmptyOrNull) {
      ...
      print("Failed");
    } else if (vendor?.vendorType.slug == "food" &&
        !isPickup &&
        (guest_count.text.isEmpty ||
            (int.tryParse(guest_count.text) ?? 0) < 3)) {
      AlertService.error(
  ...
  processOrderPlacement() async {
    ...
    checkout!.reser_guest =
        guest_count.text.isEmptyOrNull ? null : int.tryParse(guest_count.text);
    checkout!.reser_table =
        tableSelected != null ? int.tryParse(tableSelected!) : null;
    //
