...
class MultipleVendorOrderSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return VStack([
      ...
      AmountTile(
        "Subtotal".tr(),
        (subTotal ?? 0).convertIf(allowConvert).currencyValueFormat(),
      ).py2(),
      AmountTile(
        "Tax".tr(),
        "+ " + (totalTax?.convertIf(allowConvert)).currencyValueFormat(),
      ).py2(),
      AmountTile(
        "Service Fee(s)".tr(),
        "+ " + (totalFee?.convertIf(allowConvert)).currencyValueFormat(),
      ).py2(),
      Visibility(
      ...
      Visibility(
        ...
        child: VStack([
          AmountTile(
            "Driver Tip".tr(),
            "+ " +
                "$currencySymbol ${driverTip.convertIf(allowConvert)}"
                    .currencyFormat(),
          ).py2(),
          DottedLine(dashColor: context.textTheme.bodyLarge!.color!).py8(),
        ]),
    ...
    for (var i = 0; i < taxes.length; i++) {
      ...
      Widget widget =
          VStack([
            ...
            AmountTile(
              "Subtotal".tr(),
              " $currencySymbol ${vendorSubtotal.convertIf(allowConvert)}"
                  .currencyFormat(),
              amountStyle: amountStyle,
            ).py1(),
            AmountTile(
              "Tax (%s)".tr().fill(["${vendor.tax}%"]),
              " $currencySymbol ${(vendorCalTax ?? 0).convertIf(allowConvert)}"
                  .currencyFormat(currencySymbol),
              amountStyle: amountStyle,
            ).py1(),
            ...
            AmountTile(
              "",
              " $currencySymbol ${vendorSumTotalFees.convertIf(allowConvert)}"
                  .currencyFormat(currencySymbol),
              amountStyle: amountStyle,
            ).py(2),
            DottedLine(
