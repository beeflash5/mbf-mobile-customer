...
class _VendorTypeCategoriesState extends ConsumerState<VendorTypeCategories> {
  ...
  Widget build(BuildContext context) {
    final args = (vendorTypeId: widget.vendorType.id, page: 1 as int?);
    ...
    return VStack([
      Padding(
        padding:
            widget.headerPadding ?? const EdgeInsets.symmetric(horizontal: 12),
        child: HStack([
          VStack([
            if (widget.showTitle)
      ...
      if (AppStrings.categoryStyleGrid)
        CustomDynamicHeightGridView(
          padding:
              widget.listPadding ?? const EdgeInsets.symmetric(horizontal: 10),
