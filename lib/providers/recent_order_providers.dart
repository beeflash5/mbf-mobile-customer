...
class RecentOrdersController extends FamilyAsyncNotifier<List<Order>, int> {
  ...
  Future<List<Order>> build(int arg) async {
    ...
    return ref
        ...
        .getOrders(params: {if (arg != 0) 'vendor_type_id': arg});
  }
  ...
  Future<void> refresh() async {
    ...
    state = await AsyncValue.guard(
      () => ref
          .read(_orderRequestProvider)
          .getOrders(
            params: {if (_vendorTypeId != 0) 'vendor_type_id': _vendorTypeId},
          ),
    );
  }
}

final recentOrdersControllerProvider =
    AsyncNotifierProvider.family<RecentOrdersController, List<Order>, int>(
      RecentOrdersController.new,
    );
