...
class ServiceHomeState {
  ...
  ServiceHomeState copyWith({
    ...
    serviceByCategories: serviceByCategories ?? this.serviceByCategories,
  );
}

/// Service hub data per vendorTypeId (0 = none).
class ServiceHomeController extends FamilyAsyncNotifier<ServiceHomeState, int> {
  ...
  Future<ServiceHomeState> _load(int vendorTypeId) async {
    ...
    await Future.wait(
      serviceByCategories.map((cat) async {
        try {
          var services = await ref
              .read(_serviceRequestProvider)
              .getServices(queryParams: {'category_id': cat.id});
...
final serviceHomeControllerProvider =
    AsyncNotifierProvider.family<ServiceHomeController, ServiceHomeState, int>(
