...
class TodayPicksController extends FamilyAsyncNotifier<List<Product>, int> {
  ...
  Future<List<Product>> build(int arg) async {
    return ref
        ...
        .getProdcuts(
          queryParams: {if (arg != 0) 'vendor_type_id': arg, 'type': 'best'},
        );
  }
}

final todayPicksControllerProvider =
    AsyncNotifierProvider.family<TodayPicksController, List<Product>, int>(
      TodayPicksController.new,
    );
...
class NearbyVendorsHomeController
  ...
  Future<List<Vendor>> build(int arg) async {
    return ref
        ...
        .nearbyVendorsRequest(params: {if (arg != 0) 'vendor_type_id': arg});
  }
}

final nearbyVendorsHomeControllerProvider = AsyncNotifierProvider.family<
  NearbyVendorsHomeController,
  List<Vendor>,
  int
