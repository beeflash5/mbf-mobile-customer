...
class ProductReviewController
  ...
  Future<ProductReviewState> build(ProductReviewArgs arg) async {
    ...
    if (arg.summary)
      return ProductReviewState(stats: stats.stats, reviews: stats.reviews);
    final reviews = await _fetchReviews(arg.product.id, 1);
    return ProductReviewState(reviews: reviews, stats: stats.stats, page: 1);
  }
  ...
  _fetchSummary(ProductReviewArgs arg) async {
    final res = await ref
        ...
        .productReviewSummary(queryParams: {'id': arg.product.id});
    final body = res.body as Map;
    final reviews =
        (body['latest_reviews'] as List)
            .map((e) => ProductReview.fromJson(e))
            .toList();
    final stats =
        (body['rating_summary'] as List)
  ...
  Future<List<ProductReview>> _fetchReviews(int productId, int page) async {
    return ref
        .read(_productRequestProvider)
        .productReviews(page: page, queryParams: {'product_id': productId});
  }
