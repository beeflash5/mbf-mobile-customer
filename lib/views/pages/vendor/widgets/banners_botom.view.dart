...
class BannerBottom extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ViewModelBuilder<BannersViewModel>.reactive(
      ...
      onViewModelReady: (model) => model.initialiseBannerBottom(),
      builder: (context, model, child) {
        return model.isBusy
            ? LoadingShimmer().px20().h(150)
            : Visibility(
                visible: model.banneBotoms.isNotEmpty,
                child: Padding(
                    ...
                    [
                      CarouselSlider(
                        options: CarouselOptions(
                          ...
                          autoPlay: true,
                          initialPage: 1,
                          height: (!model.isBusy && model.banneBotoms.length > 0)
                      ...
                      //indicators
                      CustomVisibilty(
                        visible: model.banneBotoms.length <= 10 || showIndicators,
                        child: AnimatedSmoothIndicator(
                          activeIndex: model.currentIndex,
                          count: model.banneBotoms.length,
